...
public class Schema {
    ...
    public <T> DataModel<T> getDataModel(Type type) throws MdbException {
	if(type instanceof Class) {
	    return (DataModel<T>)objectsModels.get((Class<T>)type);
	}
	else if(type instanceof ParameterizedType || type instanceof GenericArrayType) {
	    return (DataModel<T>)multivaluedDataModels.get(getHashForType(type));
	}
	throw new MdbException(MdbErrorType.UNKNOWN_OBJECT_TYPE, new GenericNameValue("type",type));
    }
    
    public <T> DataModel<T> getDataModelByClass(Class<T> clazz){
	return (DataModel<T>)objectsModels.get(clazz);
    }
    
    public DataModel getMultivaluedDataModel(Type pt) {
