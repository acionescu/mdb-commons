...
//    }
	...
	return readAllObjects(context.getObjectDataModel(), context.getFilter(), context.getTransactionManager(),
		new ArrayList<T>());
    }

    @Override
    protected Collection<T> processAllowed(CommandContext<T> context, Collection<String> allowed) throws MdbException {
	return readObjects(allowed, context.getObjectDataModel(), context.getFilter(), context.getTransactionManager(),
		new ArrayList<T>());
    }
    ...
    protected Collection<T> processRestricted(CommandContext<T> context, Collection<String> restricted)
	    throws MdbException {
	return readAllObjectsBut(context.getObjectDataModel(), context.getFilter(), context.getTransactionManager(),
		new ArrayList<T>(), restricted);
    }
